...
class UpdateProduct(tk.Toplevel):
    ...
    def product_form(self):
        ...
        product_price_frame.grid(row=1, column=0)
    ...
    def get_product_data(self, *args):
        ...
        cursor.execute(query, (name.get(), price.get(), stock.get(), self.product_id))
        conn.commit()
        cursor.close()
    ...
    def insert_current_data(self):
        name, price, stock = App.get_product_from_id(self.root)

        self.product_name_var.set(name)
        self.product_price_var.set(price)
        self.current_value.set(stock)
